...
const CmpTextBox = (props) => {
    const { label, label2, noteRight, textPrefix, textSuffix, imageSourcePrefix, iconPrefix, iconSuffix, imageSourceSuffix, suffixColor, noteLeft, errorMessage, suffixPress, onPress, multiline, editable, absoluteSuffix, multilinestyle,secureTextEntry } = props;
        ...
        <View pointerEvents={!onPress && editable === false ? 'none' : 'auto'}>
            {(label || noteRight || noteLeft) ?
                <View style={[styles.label, { justifyContent: (noteRight && label) ? 'space-between' : noteRight ? 'flex-end' : 'flex-start' }]}>
                    {label ? <Text style={GlobalStyles.labelText}>{label}<Text style={styles.label2}>{label2}</Text></Text> : null}
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <TextInput
                            ...
                            style={[
                                ...
                                (iconPrefix || imageSourcePrefix ? { marginLeft: normalizeWidth(46) } : textPrefix ? { marginLeft: normalizeWidth(10) } : { marginLeft: normalizeWidth(20) }),
                            ...
                            placeholderTextColor={Global.color.orgGrey}
...
const styles = StyleSheet.create({
    ...
    label2: { color: Global.color.slateGrey, fontFamily: Global.font.font_family_fr },
    ...
    sectionStyle: {
        ...
        borderColor: Global.color.lightgrey66,
